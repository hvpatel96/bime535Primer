...
Adversarial attacks are a subset of techniques under adversarial machine learning that aim to fool trained machine learning models via malicious inputs. The underlying assumption that is exploited during adversarial attacks is that the model’s training and test data sets are drawn from the same statistical distribution. However, in real world application, there may exist adaptive adversarial examples that violate that assumption, leading to model manipulation and, in the worst case scenario, compromising the security of the machine learning system. Examples of common adversarial attacks include misspelling of blacklisted words or adding in whitelisted words in a spam filtering dictionary to bypass a model trained to sort emails, using faults in biometric data to impersonate a real human being in an identification model, or obfuscating malicious code within network packets to avoid intelligent malware protection. Adversarial attacks pose a real threat to any trained machine learning model and researchers have to be aware of such threats before deploying their systems in sensitive real world settings. 

Finlayson et al., in March 2019, published an interesting article titled “Adversarial attacks on medical machine learning” which acts as a good primer on existing adversarial techniques, vulnerabilities in deep learning, potential future applications of malicious intent, and the steps one can take in order to forge a better path forward. Below in just one example of adversarial attacks presented in the paper (all credit goes to [Finlayson et al. 2019](https://science.sciencemag.org/content/363/6433/1287) for the image):
...
OpenAI also has a great blog post about [Adversarial Machine Learning](https://openai.com/blog/adversarial-example-research/) where they discuss ways to defend against such attacks, including adversarial training (brute force solution where the model trains with lots of adversarial examples) and defensive distillation (where the model output are probabilities of multiple classes instead of a hard decision). However, even these training methods can be broken given enough computing power and an informed, dedicated attacker. As with all arms races, it is up to us, as informaticians, to be prepared and work closer to perfecting our systems by acknowledging the cracks and filling them in one by one.
